...
function displayArticles(articles) {
  ...
  if (articles.length === 0) {
    listEl.innerHTML = '<p style="text-align: center; color: #636e72; padding: 40px;">Nessun articolo disponibile</p>';
    return;
  }
    ...
    <div class="article-item" data-id="${article.id}">
      ...
      <div class="article-info">
        ...
        <div class="article-meta">
          <span class="meta-badge provider">${article.metadata.provider}</span>
...
function updateSelectionCount() {
  ...
  if (countNumber) {
    ...
    document.getElementById('selectionCount').textContent = `${selectedArticles.length} ${I18n.t('multi.selected')}`;
  }
  document.getElementById('startAnalysisBtn').disabled = selectedArticles.length < 2;
}

function selectAll() {
  const visibleCheckboxes = document.querySelectorAll('.article-item:not([style*="display: none"]) .article-checkbox');
...
async function startAnalysis() {
  ...
  try {
    ...
    hideProgress();
    await Modal.alert(I18n.t('multi.analysisError') + ' ' + error.message + '\n\n' + I18n.t('multi.errorDetails') + ' ' + (error.stack || 'N/A'), I18n.t('multi.errorTitle'), '❌');
  }
}
...
function showUnrelatedModal(reason = null) {
  return new Promise((resolve) => {
    ...
    const cleanup = () => {
      ...
      document.getElementById('unrelatedFullAnalysis').removeEventListener('click', handleFullAnalysis);
...
function showAnalysisModal() {
  ...
  if (currentAnalysis.globalSummary) {
    ...
    document.getElementById('tabSummary').innerHTML = '<p style="text-align: center; color: #636e72; padding: 40px;">Riassunto non generato</p>';
  }
  ...
  if (currentAnalysis.comparison) {
    ...
    document.getElementById('tabComparison').innerHTML = '<p style="text-align: center; color: #636e72; padding: 40px;">Confronto non generato</p>';
  }
  ...
  if (currentAnalysis.qa && currentAnalysis.qa.interactive) {
    ...
    if (currentAnalysis.qa.questions && currentAnalysis.qa.questions.length > 0) {
      ...
      currentAnalysis.qa.questions.forEach(qa => {
        ...
        questionEl.innerHTML = `<strong>Tu:</strong> ${qa.question}`;
        chatContainer.appendChild(questionEl);
        
        // Aggiungi risposta
        const answerEl = document.createElement('div');
        answerEl.className = 'qa-message qa-answer-msg';
        answerEl.innerHTML = `<strong>Assistente:</strong> ${qa.answer}`;
    ...
    document.getElementById('tabQA').innerHTML = '<p style="text-align: center; color: #636e72; padding: 40px;">Q&A non abilitato</p>';
  }
...
async function sendEmail() {
  ...
  try {
    ...
    if (options.includeQA && currentAnalysis.qa && currentAnalysis.qa.questions && currentAnalysis.qa.questions.length > 0) {
...
async function submitQuestion() {
  ...
  const questionEl = document.createElement('div');
  questionEl.className = 'qa-message qa-question-msg';
  questionEl.innerHTML = `<strong>Tu:</strong> ${question}`;
  chatContainer.appendChild(questionEl);
  ...
  try {
    ...
    const answerEl = document.createElement('div');
    answerEl.className = 'qa-message qa-answer-msg';
    answerEl.innerHTML = `<strong>Assistente:</strong> ${answer}`;
    chatContainer.appendChild(answerEl);
    chatContainer.scrollTop = chatContainer.scrollHeight;
    ...
    errorEl.innerHTML = `<strong>Errore:</strong> ${error.message}`;
...
async function askQuestionToArticles(question) {
  ...
  const articlesContext = currentAnalysis.qa.articles.map((a, index) => `
...
async function copyContent() {
  ...
  if (currentAnalysis.qa && currentAnalysis.qa.interactive) {
    const chatContainer = document.getElementById('qaChatContainer');
    if (chatContainer) {
      const messages = chatContainer.querySelectorAll('.qa-message:not(.qa-loading):not(.qa-welcome)');
