...
class HistoryManager {
  ...
  static async updateSummaryWithTranslation(articleUrl, translation, targetLanguage, originalLanguage) {
  ...
  static async searchHistory(query, options = {}) {
    ...
    return history.filter(entry => {
      ...
      if (searchInContent) {
        ...
        if (entry.keyPoints) {
          const keypointsMatch = entry.keyPoints.some(point => 
        ...
        // Cerca nelle Q&A
        if (entry.qa) {
          const qaMatch = entry.qa.some(qa =>
  ...
  static async clearHistory() {
    ...
    await chrome.storage.local.set({ summaryHistory: favorites });
  }
  ...
  static async clearMultiAnalysisHistory() {
    await chrome.storage.local.set({ multiAnalysisHistory: [] });
  }
  ...
  static async clearMultiAnalysisHistory() {
    ...
    // Mantieni solo i preferiti
    const favorites = history.filter(entry => entry.favorite);
    await chrome.storage.local.set({ multiAnalysisHistory: favorites });
  }
  ...
  static async savePDFAnalysis(pdfInfo, summary, keyPoints, metadata) {
    ...
    await chrome.storage.local.set({ pdfHistory: history });
  ...
  static async deletePDF(id) {
    const history = await this.getPDFHistory();
    const filtered = history.filter(entry => entry.id !== id);
    await chrome.storage.local.set({ pdfHistory: filtered });
  }
  ...
  static async clearPDFHistory() {
    ...
    // Mantieni solo i preferiti
    const favorites = history.filter(entry => entry.favorite);
    await chrome.storage.local.set({ pdfHistory: favorites });
  }
